...
- Two-sided attestations (counterparty receipt) + challenge window with optimistic resolution (F11). Highest-value new primitive.
- Multisig (Safe on Base) + non-zero Timelock delay, rehearsed on Sepolia; explicitly document the decision to **unfreeze the 0-delay policy** (frozen by choice in `PRE_MAINNET_CHECKLIST.md`).
...
Status: **code/containment work complete and verified; three owner-only items
remain** (token rotation, history rewrite + force-push, sweep decision). See
`PHASE0_OWNER_ACTIONS.md` for the exact commands.

| Item | Status | Evidence |
|---|---|---|
| 0.1 Rotate the leaked agent key | ⏳ owner action | Key moved out of `deployments.json` into `.env` (chmod 600); retirement happens at the v0.1.2 redeploy. 3 leaked keys found in history (`0x34c1f2…`, `0xff16cc…`, `0x12a8ae…`). |
| 0.2 Untrack + purge | ✅ untracked, ⏳ purge pending | 79 files (`deployments.json`, `taop.db*`, `typechain-types/**`, `*.egg-info/**`, `**/__pycache__/**`) removed from the index and gitignored; `deployments.json.example` added. Purge script written and **tested on a scratch clone** (paths gone from all commits, no key-shaped values remain, bundle backup + local-file restore work). |
| 0.3 Generated junk ignored | ✅ | `.gitignore` extended (`.venv/`, `*.egg-info/`, `*.whl`, `build/`, `*.db*`) |
| 0.4 Rotate Pinata/Replicate tokens |  owner action | Never committed (verified `git log --all -- .env` is empty), but they sat next to a published repo |

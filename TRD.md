...
The Hiring Exchange from the original TAOP vision is **not** part of the MVP. The MVP's "task" is the demo's verification event — an agent performs a verifiable off-chain task, the result is attested on-chain, and the reputation score updates. No escrow, no bidding, no milestone payments in the MVP.
...
|---|---|
| **A2A Hiring Exchange (Task Escrow Contract + bid/select/release UX)** | The biggest surface area; depends on the reputation layer proving itself first. The MVP's demo loop is the prerequisite proof. |
